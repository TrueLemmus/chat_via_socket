...
import sys
import logging
import logconfig.client_log_config
import logconfig.server_log_config
import traceback
import inspect
import socket
...
class Log:
    ...
    def __call__(self, func_to_log):
        def log_saver(*args, **kwargs):
            """Обертка"""
            ret = func_to_log(*args, **kwargs)
            real_filename = inspect.getsourcefile(func_to_log).split(sep='\\')[-1]
            LOGGER.debug(f'{real_filename} '
